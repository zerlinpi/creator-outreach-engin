...
export function validateBatch(messages: OutgoingMessage[], options: { max?: number; allowDuplicates?: boolean } = {}): OutgoingMessage[] {
  ...
  return messages.map((m) => {
    const normalized = { ...m, to: validateAddressList(m.to), cc: m.cc ? validateAddressList(m.cc) : undefined, bcc: m.bcc ? validateAddressList(m.bcc) : undefined };
    const key = `${normalized.to.join(',')}|${normalized.subject.trim().toLowerCase()}`;
    if (!options.allowDuplicates && seen.has(key)) throw new ConnectorError('RECIPIENT_REJECTED', 'Duplicate recipient and subject in the same batch.');
    seen.add(key);
    return normalized;
  });
}

export async function executeBatch(messages: OutgoingMessage[], send: (message: OutgoingMessage) => Promise<SendResult>, options: { max?: number; allowDuplicates?: boolean } = {}) {
  ...
  for (const message of validated) {
    try { results.push({ ok: true, result: await send(message) }); }
    catch (error) { results.push({ ok: false, error: toSafeError(error) }); }
  }
  return results;
}

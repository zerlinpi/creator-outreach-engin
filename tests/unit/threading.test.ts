...
describe('threading', () => {
  ...
  it('resolves exact reference chains before heuristic messages', () => {
    const base = (overrides: Partial<NormalizedMessage>): NormalizedMessage => ({ id: 'x', mailbox: 'INBOX', uid: 1, from: ['a@example.com'], to: ['b@example.com'], cc: [], subject: 'Topic', date: new Date('2026-01-01'), text: '', html: null, messageId: null, inReplyTo: null, references: [], attachments: [], ...overrides });
    const messages = [base({ id: '1', messageId: '<1@test>' }), base({ id: '2', uid: 2, messageId: '<2@test>', inReplyTo: '<1@test>', references: ['<1@test>'], date: new Date('2026-01-02') }), base({ id: '3', uid: 3, subject: 'Other', date: new Date('2026-01-03') })];
